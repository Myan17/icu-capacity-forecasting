...
def log_run(environment) -> None:
    """Aggregate Locust stats and send them to the backend API."""
    ...
    try:
        ...
        # Post directly to backend endpoint so it logs locally to the server's DB
        resp = requests.post(
            f"{host.rstrip('/')}/load-test/runs",
            json={"metrics": metrics, "tags": tags},
            timeout=5
        )
        resp.raise_for_status()

        print(f"[mlflow_reporter] Run logged to backend at {host}")
    except Exception as exc:
        print(f"[mlflow_reporter] WARNING: failed to log run to backend — {exc}")
